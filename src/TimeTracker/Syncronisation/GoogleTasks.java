package TimeTracker.Syncronisation;

import TimeTracker.TimeTracker;
import TimeTracker.Settings.ConfigSyncAccount_GoogleTasks;
...
public class GoogleTasks extends Account {
    ...
    private ConfigSyncAccount_GoogleTasks	configuration;
    
	public GoogleTasks() throws IOException {
		super();
		panelConfig = null;
		configuration = new ConfigSyncAccount_GoogleTasks();
	...
	private void initConfiguration() {
		...
		buttonResync.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				TimeTracker.getInstance().syncTasks();
			}
		});
		// Check account state
		try {
	...
	public Vector<Task> getTasks(Date updatedMin) {
		Vector<TaskCategory>	categories = this.getCategories( TimeTracker.getInstance().getCategories() );
	...
	public void createTask(Task task) {
		if (task.getCategory().getSyncId(accountIdent) == null) {
			...
			TimeTracker.getInstance().saveCategories();
		}
		try {
	...
	public void load(InputStream stream) {
		try {
			...
			configuration = (ConfigSyncAccount_GoogleTasks) objStream.readObject();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
